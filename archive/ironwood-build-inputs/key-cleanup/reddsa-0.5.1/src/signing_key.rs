...
/// Clears the secret scalar, retaining the cached public verification key.
///
/// Discard the signing key after clearing it: its scalar and public key no longer
/// correspond. This does not clear other copies or signing temporaries.
#[cfg(feature = "zeroize")]
impl<T: SigType> zeroize::Zeroize for SigningKey<T>
where
    T::Scalar: zeroize::Zeroize,
{
    fn zeroize(&mut self) {
