...
/// Clears this key's secret scalar, but not its cached public verification key.
/// Discard the key after clearing it; other copies and signing temporaries remain.
#[cfg(feature = "zeroize")]
impl<T: SigType> zeroize::Zeroize for SigningKey<T>
where
    reddsa::SigningKey<T>: zeroize::Zeroize,
{
    fn zeroize(&mut self) {
