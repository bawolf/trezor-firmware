...
pub mod v2 {
    ...
    impl Pczt {
        ...
        /// Parses a PCZT from its v2 encoding.
        ///
        /// Returns the logical [`super::Pczt`]. Input in any other container
        /// version (including v1) fails with [`super::ParseError::UnknownVersion`].
        /// This is the exact v2 decoding used by [`super::Pczt::parse`], which
        /// delegates its v2 arm to this method, so the two entry points cannot
        /// diverge. It restricts only the PCZT container version, not the
        /// transaction version or the permitted pools; callers must still
        /// validate the transaction's contents.
        ///
        /// The caller must already have bounded the input's length and shape:
        /// like [`super::Pczt::parse`], decoding is `postcard`/`serde`-derived
        /// and may preallocate collection buffers of up to 1 MiB from length
        /// prefixes in unvalidated bytes before the input is known to be well
        /// formed. Do not hand either entry point untrusted input without an
        /// external size and structure limit.
        ///
        /// Trailing data after a well-formed encoding is handled identically to
        /// [`super::Pczt::parse`]: the trailing bytes are accepted and ignored.
        /// Callers that require exact-length input must enforce that bound
        /// themselves before parsing.
        ///
        /// Note on code size: this method never decodes the v1 encoding, so
        /// binaries whose only parsing entry point is this method need not
        /// carry the v1 decoder; whether it is actually omitted depends on
        /// the linker and build configuration.
        pub fn parse(bytes: &[u8]) -> Result<super::Pczt, super::ParseError> {
            let (version, body) =
                crate::parse_header(bytes, crate::MAGIC_BYTES).map_err(|e| match e {
                    crate::HeaderParseError::InvalidMagic => super::ParseError::NotPczt,
                    crate::HeaderParseError::TooShort => super::ParseError::TooShort,
                })?;
            if version != crate::PCZT_VERSION_2 {
                return Err(super::ParseError::UnknownVersion(version));
            }
            postcard::from_bytes::<Self>(body)
                .map_err(super::ParseError::Invalid)
                .and_then(Self::into_logical)
        }
    }
    ...
    mod tests {
        ...
        use super::Pczt;
        use crate::{ParseError, orchard::NoteVersion, roles::creator::Creator};

        #[test]
        fn parse_round_trips_v2_and_rejects_other_container_versions() {
            let logical = Creator::new(BranchId::Nu6_3.into(), 10_000_000, 133, None, None)
                .unwrap()
                .build()
                .unwrap();
            let bytes = Pczt::try_from(logical).unwrap().serialize();

            // The v2-only decoder returns the logical PCZT, and re-encoding it
            // reproduces the container.
            let parsed = Pczt::parse(&bytes).unwrap();
            let reencoded = Pczt::try_from(parsed).unwrap().serialize();
            assert_eq!(reencoded, bytes);

            // A v1 container is rejected with the exact version, and a wrong
            // magic is rejected as not a PCZT.
            let v5 = Creator::new(BranchId::Nu6.into(), 10_000_000, 133, None, None)
                .unwrap()
                .build()
                .unwrap();
            let v1_bytes = crate::v1::Pczt::try_from(v5).unwrap().serialize();
            assert!(matches!(
                Pczt::parse(&v1_bytes),
            ...
            bad_magic[0] ^= 1;
            assert!(matches!(Pczt::parse(&bad_magic), Err(ParseError::NotPczt)));
        }
...
impl Pczt {
    ...
    /// Parses a PCZT from its encoding, accepting every known container version.
    ///
    /// The v2 encoding is decoded by delegating to [`v2::Pczt::parse`], which can
    /// also be called directly to accept only the v2 encoding.
    pub fn parse(bytes: &[u8]) -> Result<Self, ParseError> {
        ...
        match version {
            ...
            PCZT_VERSION_2 => v2::Pczt::parse(bytes),
    ...
    /// directly; [`v2::Pczt::parse`] is the corresponding version-restricted
    /// decoder.
    #[inline(never)]
