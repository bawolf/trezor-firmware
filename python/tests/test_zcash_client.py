...
The host never parses a PCZT, so the PCZTs are synthetic bytes at the lengths
that matter for chunking; these tests exercise the transfer protocol and the
signature-record response only, never signing.
"""
...
# Synthetic PCZTs keyed by the number of actions a real one of that length would
# hold, at the sizes that matter for the transfer: two, three and ten chunks of
# CHUNK bytes. The host never parses a PCZT, so any bytes serve; these are
# deterministic so a transcript is reproducible.
PCZT_LENGTHS = {1: 1236, 2: 2429, 8: 10037}


def _pczt(actions: int) -> bytes:
    return bytes((i * 7 + actions) % 256 for i in range(PCZT_LENGTHS[actions]))
...
def test_export_viewing_key_loads_a_response_without_the_fingerprint() -> None:
    """The fingerprint is optional: a response carrying only the key loads."""
    key = VIEWING_KEYS[MAINNET]
    buf = BytesIO()
    protobuf.dump_message(
        buf, messages.ZcashViewingKey(key=key, seed_fingerprint=SEED_FINGERPRINT)
    )
    with_fingerprint = buf.getvalue()
    # field 2, wire type 2 (0x12), length 32 (0x20), then the fingerprint.
    assert with_fingerprint.endswith(b"\x12\x20" + SEED_FINGERPRINT)
    encoded = with_fingerprint[: -(2 + len(SEED_FINGERPRINT))]
...
@pytest.mark.parametrize("actions, chunks", [(1, 2), (2, 3), (8, 10)])
def test_sign_pczt_chunk_counts(actions: int, chunks: int) -> None:
    pczt = _pczt(actions)
    session = scripted(*sign_script(pczt, records_for([0])))
    zcash.sign_pczt(session, pczt, MAINNET, 0, REFERENCE_HEIGHT)
    acks = [m for m in sent(session) if isinstance(m, messages.ZcashPcztAck)]
    assert len(acks) == chunks
...
def test_cancel_after_violation_keeps_the_session_synchronized() -> None:
    ...
    with pytest.raises(exceptions.ProtocolError):
        zcash.sign_pczt(session, _pczt(1), MAINNET, 0, REFERENCE_HEIGHT)
