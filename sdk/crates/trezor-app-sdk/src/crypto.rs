...
///
/// The spending key is overwritten with zeros when the value is dropped. Copies
/// the compiler makes when moving the value are not, and neither is the IPC
/// receive buffer the key arrived in.
pub struct Zip32OrchardAccount {
    ...
    /// Whether wallets should warn about the backup, as ZIP 315 asks: a 12- or
    /// 18-word BIP-39 mnemonic, or a 128-bit SLIP-39 secret.
...
/// Fills `buffer` with random bytes from the device's random number generator.
