...
const EXTAPP_TOOL: &str = "core/tools/trezor_core_tools/extapp_tool.py";
...
///
/// TODO: temporary. Finds `extapp_tool.py` in an ancestor of the workspace
/// root, so it works only inside a trezor-firmware checkout; elsewhere proof
/// generation is skipped with a warning. To be replaced by trezorctl.
pub fn generate_app_proofs(model: Model, emulator: bool) -> Result<()> {
    let workspace_root = helpers::root_dir()?;
    let Some(repo_root) = workspace_root
        .ancestors()
        .find(|dir| dir.join(EXTAPP_TOOL).is_file())
        .map(Path::to_path_buf)
    else {
        println!(
            "xtask: warning: not running inside a trezor-firmware checkout, skipping app proof/RootPacket generation"
        );
        return Ok(());
    };
    ...
    cmd.arg("run")
        .arg(repo_root.join(EXTAPP_TOOL))
    ...
    let status = cmd
        .status()
        .context("Failed to spawn `extapp_tool.py`")?;
    ensure!(
        status.success(),
