...
//! (design document, see the crate docs, §2 and §4).
//!
...
/// Inner personalization of the consent token's byte commitment (design §4).
...
/// One retained action (design §2 `ActionRecord`).
...
/// One signature record of the response message (design §1).
...
struct Slot {
    ...
    /// the heap so `review_stream` never builds `Pending` by value on the
    /// stack.
...
/// linger in the freed heap block on every
...
/// Per-stream state after the header (design §2 "Action `i`").
struct Body {
    digest: Digest,
...
struct Stream {
    ...
    /// heap and is only ever reached through this pointer.
...
pub struct Session<R> {
    ...
    session: [u8; 32],
    counter: u64,
    /// Boxed: the CAP-sized `Stream`/`Body` live in the heap
...
impl<R: RngCore + CryptoRng> Session<R> {
        ...
        while consumed < chunk.len() {
            ...
            let trailer = match item {
                ...
                Some(Item::Header(header)) => {
                    ...
                    // `Body::new` returns a `Box<Body>` built in the heap, so the
    ...
    /// Trailer, VerifyDummies and Totals of design §2, then the token. The
    ...
    fn review_stream(&mut self, trailer: Trailer) -> Result<Review> {
        ...
        // the wiped classifier) stay in the heap and drop in place when the
...
impl Body {
    ...
    /// `Box<Body>` built in the heap and stays out of line so
    ...
    /// Design §2 steps 1-9: the per-action checks of [`crate::verify_bundle`]
    ...
    fn action(
        ...
        // Design §2 step 3. For a real spend `Spend::parse` would derive the
