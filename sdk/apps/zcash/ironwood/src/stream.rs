...
//! buffer, so the device never holds more than one action's bytes; see the
//! design document (crate docs) §1-2.
//!
...
// scanner's single buffer and therefore a line item in the heap
...
// against the heap, and update the literal in the same commit.
const _: () = assert!(
    HEADER_BUDGET == 101
...
impl<'a> Reader<'a> {
    ...
    /// the sighash: admitted within [`USER_ADDRESS_BUDGET`] and required to be
    /// UTF-8 as postcard's `String` is.
...
/// check; `user_address` is admitted, bounded and ignored: the handler shows
/// the address it solved from `scriptPubKey`.
fn transparent_output<'a>(r: &mut Reader<'a>) -> Parse<TransparentOutput<'a>> {
    let value = r.varint()?;
    malformed(value <= MAX_MONEY)?;
    // Every transparent output here is a payment (§8), and a payment of
