...
#![forbid(unsafe_code)]
#![deny(clippy::all)]
//! Bounded Ironwood PCZT approval core for the Zcash application.
//!
//! The rules this crate enforces are written down in the design document
//! `docs/common/zcash-ironwood-signing.md` of bawolf/trezor-firmware@7864a22444,
//! which is not in this tree; the "§N" in the comments below are its sections.
//!
//! The host selects the network, account, and reference height. The device must
//! validate those values, derive the account from its own seed, and bind all
//! three to consent; the reference height is a policy input and is not shown.
//! Fee and expiry limits are device-owned policy. This crate retains no seed or
//! separate full viewing-key object.

#[cfg(all(feature = "test", target_os = "none"))]
compile_error!("the `test` feature is host-only and must not enter a bare-metal build");
...
/// Byte budget of a text memo the device shows verbatim (design §7: display
...
/// What the device shows for a payment output's memo (design §7).
///
...
/// allocation comes out of the device's small fixed heap, so boxing the memo
/// would trade 258 bytes of enum size for a heap allocation per reviewed
...
/// `tests/stream_equivalence.rs`), never a device path: the device calls
/// `Session` exclusively. Host-only, so the device image carries one
...
fn validate(
    ...
    Ok(Validated {
        ...
        // still clones the Ironwood bundle internally; this whole-PCZT path is
        // the host-only reference, so that allocation never reaches the device.
