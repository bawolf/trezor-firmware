...
pub fn decode_typed_data(data: &[u8], type_name: &str) -> Result<String> {
    if type_name.starts_with("bytes") {
        ...
        return address_from_bytes(data, None).c();
...
pub fn get_fee_items_regular(
    ...
    let gas_price_str = format_ethereum_amount(gas_price, None, network, true).c()?;
...
pub fn get_fee_items_eip1559(
    ...
    let gas_limit_str = uformat!("{} units", gas_limit.to_string().as_str());
    let max_gas_fee_str = format_ethereum_amount(max_gas_fee, None, network, true).c()?;
    let max_priority_fee_str = format_ethereum_amount(max_priority_fee, None, network, true).c()?;
    Ok([
        (tr!("ethereum__gas_limit").into(), gas_limit_str, false),
        (
...
pub fn get_data_confirmer(total_len: usize) -> Box<dyn FnMut(&[u8]) -> Result<()>> {
    ...
    Box::new(move |chunk: &[u8]| {
        if first {
            ...
            if skip {
                let mut pb = get_progress_indicator(total_len, 0).c()?;
                return pb(chunk);
            }
        }
        ...
        loop {
            ...
            match prefix_len {
                None => {
                    let mut pb = get_progress_indicator(total_len, confirmed_len).c()?;
                    pb(remaining)?;
