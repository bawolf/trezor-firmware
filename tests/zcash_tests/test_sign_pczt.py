...
def _accept_flow_recording_addresses(session: Session, payments: int, shown: list):
    br = yield
    assert br.code == B.Warning
    session.debug.press_yes()
...
def test_payment_shows_its_user_address(session: Session) -> None:
...
def test_long_user_address_is_shown_whole(
    session: Session, unknown_receiver_bytes: int | None
) -> None:
    ...
    for payment in result["payments"]:
        ...
        address = unified_address(receivers, "u")
...
def test_user_address_for_another_receiver_is_refused(session: Session) -> None:
    ...
    with (
        ...
        pytest.raises(TrezorFailure, match="does not match the Orchard receiver"),
    ):
