...
typedef struct {
  ...
  bool (*syslog_start_record)(const log_source_t* source, uint32_t level);

  ssize_t (*syslog_write_chunk)(const char* text, size_t text_len,
                                bool end_record);
  ...
  // Appended so that the offsets of the fields above do not move.
