...
pub fn configure_cargo(args: &ResolvedBuildArgs, cmd: &mut process::Command) -> Result<()> {
    ...
    if rebuild_std {
        if args.zcash_shielded && matches!(args.project, crate::args::Project::Firmware) {
...
mod tests {
    ...
    use crate::options::{BuildOptions, ZCASH_SHIELDED_MODELS, zcash_shielded_unsupported_message};

    fn zcash_shielded_build_args(project: Project, model: Model) -> BuildArgs {
        BuildArgs {
            project,
            model,
            emulator: false,
    ...
    #[test]
    fn omits_zcash_shielded_by_default() {
        ...
        assert!(!features.contains(&"zcash_shielded".to_string()));
    }

    #[test]
    fn enables_zcash_shielded_for_t3t1_firmware_emulator() {
        let args = ResolvedBuildArgs {
            model: Model::T3T1,
            emulator: true,
            zcash_shielded: true,
    ...
    /// layout its model config selects. Derived from `ZCASH_SHIELDED_MODELS`,
    /// so a new model needs no edit here.
    #[test]
    fn enables_zcash_shielded_for_every_allowed_model() {
        for &model in ZCASH_SHIELDED_MODELS {
            let args = ResolvedBuildArgs {
                model,
                zcash_shielded: true,
                frozen: true,
            ...
            let features = resolve_features(&args).unwrap().features;
            assert!(
                features.contains(&"zcash_shielded".to_string()),
                "{} must build with zcash_shielded",
                model.model_id()
            );
        }
    }
    ...
    fn allowed_models_cover_caesar_delizia_and_eckhart() {
        for (model, layout) in [
            ...
            assert!(
                ZCASH_SHIELDED_MODELS.contains(&model),
    ...
    /// Every firmware model not on the allow-list is refused, the D00x devkits
    /// included.
    #[test]
    fn rejects_zcash_shielded_for_other_models() {
        let rejected: Vec<Model> = Model::value_variants()
            .iter()
            .copied()
            .filter(|model| !ZCASH_SHIELDED_MODELS.contains(model))
            .collect();
        assert!(!rejected.is_empty());
        for model in rejected {
            let error = ResolvedBuildArgs::from_build_args(&zcash_shielded_build_args(
                Project::Firmware,
                model,
            ))
            .unwrap_err();
            assert_eq!(error.to_string(), zcash_shielded_unsupported_message());
        }
    }

    /// `core/Makefile` passes the option to every target it builds, so the
    /// boardloader, bootloader and kernel must accept it and build unchanged.
    #[test]
    fn ignores_zcash_shielded_for_other_projects() {
        for project in [Project::Boardloader, Project::Bootloader, Project::Kernel] {
            let args = ResolvedBuildArgs::from_build_args(&zcash_shielded_build_args(
                project,
                Model::T3T1,
            ))
            .unwrap();
    ...
    /// have to guess which targets `--zcash-shielded` accepts.
    #[test]
    fn the_rejection_message_lists_every_allowed_model() {
        let message = zcash_shielded_unsupported_message();
        for model in ZCASH_SHIELDED_MODELS {
            assert!(
    ...
    #[test]
    fn rejects_zcash_shielded_for_bitcoin_only_firmware() {
        let mut args = zcash_shielded_build_args(Project::Firmware, Model::T3T1);
        args.options.btc_only = Some(true);
        let error = ResolvedBuildArgs::from_build_args(&args).unwrap_err();
        assert_eq!(
            error.to_string(),
            "--zcash-shielded cannot be combined with --btc-only"
        );
    }

    #[test]
    fn omits_zcash_shielded_from_firmware_dependency_builds() {
        let firmware_args = ResolvedBuildArgs::from_build_args(&zcash_shielded_build_args(
            Project::Firmware,
            Model::T3T1,
        ))
        .unwrap();
        let firmware_features = resolve_features(&firmware_args).unwrap().features;
        assert!(firmware_features.contains(&"zcash_shielded".to_string()));
    ...
    #[test]
    fn accepts_explicitly_disabled_zcash_shielded_for_unsupported_targets() {
        let args = BuildArgs {
            project: Project::Kernel,
            model: Model::T3W1,
            emulator: false,
