...
fn every_admitted_shape_signs_in_three_quarters_of_the_scratch_tier() {
    ...
    let cases = [
        ("2 actions", build_wide(2)),
        ("8 actions", build_wide(8)),
        ("16 actions", build_wide(16)),
        ("32 actions", build_wide(MAX_ACTIONS)),
        (
            "1 action + 31 transparent",
            build_one_action_deshield(&transparent_values(MAX_TRANSPARENT_OUTPUTS)),
        ),
    ];
