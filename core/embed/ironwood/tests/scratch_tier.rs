...
//!
//! It traces what `signing.rs` builds -- the boxed request and the key
//! derivation around the `Session`, not a bare `Session` -- through the real
//! first-fit free list (`arena.rs`, included by path), and reports the furthest
//! byte any session reached. First-fit makes the same choices in any tier at
//! least that long, so that figure is the smallest tier the session runs in,
...
use common::*;
use ironwood::{MAX_ACTIONS, USER_ADDRESS_BUDGET};
...
/// Every action's `user_address` at its budget: each payment confirmation then
/// carries the longest string the session copies out for the handler.
fn with_longest_user_addresses(pczt: Vec<u8>) -> Vec<u8> {
    let mut value = json(&pczt);
    for action in value["ironwood"]["actions"].as_array_mut().unwrap() {
        action["output"]["user_address"] = "u".repeat(USER_ADDRESS_BUDGET).into();
    }
    encode(value)
}
...
fn every_admitted_shape_signs_in_three_quarters_of_the_scratch_tier() {
    ...
    let cases = [
        ...
        (
            "32 actions, longest addresses",
            with_longest_user_addresses(build_actions(MAX_ACTIONS)),
        ),
    ...
    for (label, pczt) in &cases {
        ...
        println!(
            "  {label:30} {:5} B PCZT: reach {reach:5} B, peak {peak:5} B in use, margin {margin:.1} %, {refused} refused",
            pczt.len()
        );
