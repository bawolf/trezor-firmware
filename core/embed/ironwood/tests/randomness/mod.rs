...
fn unavailable_session_entropy_prevents_engine_creation() {
    assert_error(
        Engine::with_rng(policy(Network::Testnet, 100_000), SessionUnavailable)
            .err()
            .unwrap(),
        ErrorCode::Entropy,
    );
...
fn seeded_synthetic_signing_is_reproducible_and_preserves_metadata() {
    ...
    for _ in 0..2 {
        ...
        let signed_bytes = signed.serialize().unwrap();
...
fn independently_seeded_sessions_reject_each_others_tokens() {
    ...
    .unwrap();
    let a = first.begin_test(&bytes).unwrap();
    let b = second.begin_test(&bytes).unwrap();
...
fn entropy_panic_after_first_signature_consumes_consent_without_response() {
    ...
    .unwrap();
    let review = engine
        .begin_test(&build_inputs(&[100_000, 100_000]))
        .unwrap();
    engine.approve(review.token()).unwrap();
