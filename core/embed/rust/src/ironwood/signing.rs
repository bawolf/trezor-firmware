...
//! (docs/common/zcash-ironwood-signing.md §2; the handler it serves is §6).
//!
//! The MicroPython module `trezorzcash` drives exactly one request at a
...
/// Pool tag of a signature record (design §1).
...
pub struct Output {
    ...
    pub receiver: [u8; 43],
    pub value: u64,
    /// What to show for the memo (§7: text within the display budget
    /// verbatim, anything else as a hash), recovered from the signed
    /// ciphertext of this action.
    pub memo: Memo,
    /// The wallet's untrusted recipient string; the handler shows it only if
    /// its Orchard receiver is `receiver` (design §7).
