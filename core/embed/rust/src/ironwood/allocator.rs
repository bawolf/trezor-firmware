...
//! first use; docs/common/zcash-ironwood-signing.md §5). None of it may live in
...
/// margin. A 24 KiB tier refuses a 2-action sign by 432 B once the boxed
/// request and the key derivation around it are counted, and it is packed at
/// the refusal, not fragmented -- in-place `realloc` is what keeps it that way.
/// The tier is borrowed from the GC heap only while
...
// them through `trezorzcash.debug_region_info()` and nothing in production
...
pub fn forbid_installed_scratch() {
    if arenas().scratch_installed() {
        rtl::system_exit_fatal("Zcash scratch not released", file!(), line!());
    }
}
...
/// is collected -- the mirror image of what the rooted tier exists to prevent,
/// and fatal rather than undefined.
pub fn release_scratch() {
    if arenas().release_scratch().is_err() {
        rtl::system_exit_fatal("Zcash scratch retained", file!(), line!());
    }
}
...
fn served(payload: *mut u8) -> *mut u8 {
    if payload.is_null() {
        rtl::system_exit_fatal("Zcash allocation failed", file!(), line!());
    }
    #[cfg(feature = "debuglink")]
    mark_peaks();
    payload
