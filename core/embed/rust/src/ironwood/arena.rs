...
impl Arenas {
    ...
    /// collected. That is the mirror image of what the rooted tier exists to
    /// prevent, and the caller must treat it as fatal. The span is
...
mod tests {
    ...
    fn a_second_session_starts_from_the_same_rooted_tier() {
        ...
        for buffer in [&mut first, &mut second] {
            ...
            assert!(arenas.release_scratch().is_ok());
            // The rooted tier is untouched by a session, twice over: nothing a
            // session rooted may survive it.
