...
from apps.zcash import helpers
...
class TestZcashAccount(unittest.TestCase):
    def test_network_account_and_path_policy(self):
        self.assertEqual(
            helpers.validate_network_account(ZcashNetwork.Mainnet, 0),
            ("Zcash", "Mainnet", 133),
        )
        self.assertEqual(
            helpers.validate_network_account(ZcashNetwork.Testnet, 0x7FFF_FFFF),
            ("Zcash Testnet", "Testnet", 1),
        )
        self.assertEqual(helpers.account_path(133, 7), "m/32'/133'/7'")
        self.assertEqual(helpers.account_label(7), "ZEC #8")

        for network, account in ((True, 0), (ZcashNetwork.Mainnet, False)):
            with self.assertRaises(wire.DataError) as raised:
                helpers.validate_network_account(network, account)
    ...
    def test_diversifier_policy(self):
        helpers.validate_diversifier_index(bytes(11))
        for invalid in (bytes(10), bytes(12)):
            with self.assertRaises(wire.DataError) as raised:
                helpers.validate_diversifier_index(invalid)
            self.assertEqual(raised.value.message, "Malformed Zcash request")

        helpers.validate_diversifier_index(memoryview(bytes(11)))

    @unittest.skipUnless(utils.USE_ZCASH_SHIELDED, "requires USE_ZCASH_SHIELDED")
    ...
    def test_weak_backup_predicate_is_memoized(self):
        ...
        with patch(mnemonic, "get_type", lambda: BackupType.Bip39):
            with patch(mnemonic, "get_secret", counting_secret):
                self.assertTrue(helpers.has_weak_backup())
                self.assertTrue(helpers.has_weak_backup())
                self.assertTrue(helpers.has_weak_backup())
    ...
    def test_legacy_session_identity(self):
        ...
        with patch(utils, "USE_THP", False):
            with patch(context, "get_context", lambda: ctx):
                snapshot = helpers.snapshot_session()
                helpers.require_session(snapshot)
