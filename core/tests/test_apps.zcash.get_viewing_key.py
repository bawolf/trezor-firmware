...
if not utils.BITCOIN_ONLY:
    ...
    from apps.zcash import get_viewing_key, helpers, unified_addresses
...
class TestZcashGetViewingKey(unittest.TestCase):
    def setUp(self):
        self.patchers = []
        self.calls = []
        ...
        self._patch(utils, "USE_ZCASH_SHIELDED", True)
        ...
        self._patch(helpers, "has_weak_backup", lambda: False)
    ...
    def test_network_account_consent_and_exact_key(self):
        ...
        for network, account, expected, details in vectors:
            ...
            self.assertEqual(confirmation[1][0], "zcash_export_viewing_key")
    ...
    def test_seed_fingerprint_is_exported_only_behind_its_own_warning(self):
        ...
        self.assertEqual(warning["br_name"], "zcash_seed_fingerprint")
    ...
    def test_weak_backup_warning_follows_privacy_consent(self):
        ...
        self._patch(helpers, "has_weak_backup", lambda: True)
        await_result(get_viewing_key.get_viewing_key(self._message()))
        self.assertEqual(
            [call[0] for call in self.calls],
    ...
    def test_direct_feature_disabled_invocation_fails_closed(self):
        self._patch(utils, "USE_ZCASH_SHIELDED", False)
        with self.assertRaises(wire.ProcessError) as raised:
            await_result(get_viewing_key.get_viewing_key(self._message()))
        self.assertEqual(raised.value.message, "Zcash shielded support is not enabled")
        self.assertEqual(self.calls, [])

    def test_handler_registration_is_feature_gated(self):
        with patch(utils, "BITCOIN_ONLY", False):
            with patch(utils, "USE_ZCASH_SHIELDED", True):
                self.assertEqual(
            ...
            with patch(utils, "USE_ZCASH_SHIELDED", False):
