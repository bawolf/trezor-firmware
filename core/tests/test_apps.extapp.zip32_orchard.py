...
class TestZip32OrchardDerivation(unittest.TestCase):
    def test_zip32_orchard_vectors(self):
        ...
        # orchard's src/test_vectors/zip32.rs)
        seed = bytes(range(32))
        vectors = [
    ...
    def test_account_key(self):
        # "abandon" x11 + "about", mainnet account 0; equals orchard's
    ...
    def test_seed_length(self):
        for length in (0, 15, 17, 31, 253):
            with self.assertRaises(ValueError):
                check_seed_length(bytes(length))
...
class TestZip32OrchardPolicy(unittest.TestCase):
    ...
    def test_refused_without_the_curve(self):
...
class TestZip32OrchardApprovals(unittest.TestCase):
    def test_approval_is_per_app_instance_and_account(self):
        app = app_instance(7, bytes(32))
        other_instance = app_instance(8, bytes(32))
        other_image = app_instance(7, b"\x01" * 32)

        approvals = with_approval(None, app, 133, 0)
        self.assertTrue(is_approved(approvals, app, 133, 0))
        self.assertFalse(is_approved(approvals, app, 133, 1))
        self.assertFalse(is_approved(approvals, app, 1, 0))
        self.assertFalse(is_approved(approvals, other_instance, 133, 0))
        self.assertFalse(is_approved(approvals, other_image, 133, 0))
        self.assertFalse(is_approved(None, app, 133, 0))

        approvals = with_approval(approvals, app, 1, 0)
        self.assertTrue(is_approved(approvals, app, 133, 0))
        self.assertTrue(is_approved(approvals, app, 1, 0))

        # Another app starts from nothing.
        approvals = with_approval(approvals, other_image, 133, 1)
        self.assertFalse(is_approved(approvals, app, 133, 0))
        self.assertFalse(is_approved(approvals, other_image, 133, 0))
        self.assertTrue(is_approved(approvals, other_image, 133, 1))

    def test_keeps_the_newest_eight(self):
        app = app_instance(7, bytes(32))
        approvals = None
        for account in range(10):
            approvals = with_approval(approvals, app, 133, account)
        self.assertEqual(len(approvals), 36 + 8 * 5)
        for account in range(10):
            self.assertEqual(is_approved(approvals, app, 133, account), account >= 2)
...
class TestZip32OrchardWeakBackup(unittest.TestCase):
    ...
    def test_slip39(self):
        for backup_type in (
            ...
            self.assertTrue(has_weak_backup(bytes(24), backup_type))
