...
async def run(request: ExtAppMessage) -> ExtAppResponse:
    ...
    while True:
        ...
        try:
            ...
            # A task the kernel killed (a fault, e.g. a stack overflow) sends
            # nothing either; tell it from one that stayed silent.
            if not image.is_running():
                raise DataError(f"Task stopped: {request.instance_id}")
        ...
        if service == _SERVICE_UI:
            try:
                ...
                die(DataError(f"Invalid UI request: {e}"))
            ...
            try:
                ...
                # E.g. a message larger than the app's inbox: the app would
            ...
            try:
                ...
                die(DataError(f"Invalid progress request: {e}"))
