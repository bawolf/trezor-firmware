# Zcash shielded signing

The shielded Zcash app signs the Ironwood actions of a PCZT the device never
holds in full: the host uploads it in 1,024-byte chunks, an incremental scanner
consumes the v2 encoding section by section, and the device returns detached
spend authorization signatures. This document records the rules the implementation
enforces, so that the comments in `core/embed/ironwood`,
`core/embed/rust/src/ironwood/` and `core/src/apps/zcash` can cite a rule by
number instead of restating it; "§N" in those comments is a section of this
document.
...
| Action `i` | the action grammar, about 2 KB | one `ActionRecord` (§2) |
...
`Features.capabilities`, which only firmware built with `--zcash-shielded` reports.
...
capability. CI runs the device tests, including the capability check, on
T3B1, T3T1 and T3W1 emulators built with `--zcash-shielded`.
...
transparent bundle may be absent, or present under the rules of §8.

**Transparent output `j`.** Each admitted output is fed to the ZIP-244 T.2c
outputs hash (§3), added to the transparent total, and shown now with its
...
5. A `zip32_derivation` on the spend or the output is admitted only when it
   names the device's own seed fingerprint (§7) *and* the consented account
...
then is the consent token derived (§4).
...
fed. With no transparent **inputs** -- which §8 makes structural -- ZIP-244
...
**Boundary.** The scratch tier is a Python object: for the length of a session the signing state,
...
`core/src/apps/zcash/sign_pczt.py` drives the native session in one async
loop and renders every screen through the
...
what it may show: a 43-byte receiver, the wallet's `user_address` for it
(untrusted, checked per §7), a value, and either the
text of a memo within the display budget or its 32-byte digest. The full
memo bytes never cross into Python.

## 7. Display and admission policy
...
both halves. Those per-output confirmations are not consent (§2).

**Recipient address.** A payment is shown under the address the user can
compare with the one they entered. The wallet's `output.user_address` is
untrusted and outside the sighash, so the handler shows it only if it decodes,
for this network, to a unified address whose Orchard receiver is exactly the
receiver the device verified; its other receivers are neither checked nor
paid. A `user_address` that fails that check refuses the PCZT (`DataError`).
Without one, the device shows the Orchard-only unified address of the verified
receiver. This follows `apps/zcash/signer.py` (`output_derive_script`), which
decodes a unified address entered for a transparent `SignTx` payment, pays its
transparent receiver and shows the whole address. A PCZT's transparent outputs
do not use it (§8).
...
`zip32_derivation` carries and the value §2 step 5 compares against. It is a
...
- **zero inputs.** An input is `Policy`. This is not a parse convenience: zero
  transparent inputs is what makes ZIP-244 §S.2 collapse to §T.2 (§3), and the
...
| `user_address` | admitted within `USER_ADDRESS_BUDGET`, required to be UTF-8, and **ignored**: the device shows the Base58Check address it solves from `scriptPubKey`, which is what a transparent recipient enters |
